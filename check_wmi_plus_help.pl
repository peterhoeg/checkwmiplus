...
REQUIRED OPTIONS FOR WMIC Server
 These only apply if using the new WMIC Server
 You must specify username/password/domain (referred to as auth info) in the WMIC Server configuration. Check WMI PLus does not hold auth info. It only points to the auth info in the WMIC Server.
 You must point to the auth info using an identifier that matches one in the WMIC Server configuration.
 -u ID  specify the identifier for the auth info that matches the one defined in the WMIC Server

 -p TOKEN  specific the token required to authenticate to the WMIC Server. This is not Windows authentication information. The tokens are defined in the WMIC Server configuration

 -A IDFILE  the full path to an ID file. The file is read by Check WMI PLus. You can override the settings in this file by using -u ID -p TOKEN. The IDFILE should contain only two lines as follows:

      <ID>
      <TOKEN>

 where <ID> matches the ID required by the WMIC Server and <TOKEN> (optional) matches the TOKEN required by the WMIC Server. It is valid to omit the <TOKEN> line.
 The legacy WMIC client AUTHFILE format is also supported.

REQUIRED OPTIONS FOR LEGACY WMIC CLIENT
 These only apply if using the legacy wmic client
...
LESS COMMONLY USED OPTIONS
 ...
 --IgnoreAuthFileWarnings  Ignores warnings about problems reading authentication files (specified using the -A option). Applies only when also using --legacywmicclient
 ...
 --legacywmicclient  use the legacy wmic client ie the single binary command "wmic".
